...
const { BrowserWindowPool } = require('./pool');
...
class WinsManager {
  constructor (props) {
    ...
    this._poolInstance = new BrowserWindowPool(this.browserConfig);
    /**
  ...
  createWithPool (config = {}) {
    const instance = this._poolInstance.getBrowserWindow(config);
   ...
   */
  create (config = {}, position = null) {
    assert(config.name, 'You must set [name] in config when using create');
  ...
  setPosition (config, position) {
    ...
    if (position.immediate) {
      setBoundsByPosition(
        position,
        this.getInstance(config.parentName),
        winInstance
      );
    }
   ...
   */
  __registerEventFunction (winName, eventName, fn, relation) {
    this.getInstance(winName).on(eventName, fn);
    if (relation) fn.__relation = relation;
    update(this._eventFunctions, `[${winName}][${eventName}]`, (v) =>
      Array.isArray(v) ? [...v, fn] : [fn]
    );
    return this;
  }
   ...
   * @returns
   */
  __cancelEventFunction (winName, eventName, relation) {
   ...
   * @param {*} name
   */
  freshExtraInfo (winName, options = {}) {
    const instance = this.getInstance(winName);
    if (!instance) return;
    // boundsInfo
    if (!instance.bounds) {
  ...
  setCustomBounds (winName, bounds) {
    const instance = this.getInstance(winName);
    if (!instance) return;
  ...
  addDevTools (name, option = {}) {
    ...
    if ((IS_DEV && devValid) || (!IS_DEV && prodValid)) {
      require('electron-localshortcut').register(instance, hotKey, () =>
